...
Transitive dependency vulnerabilities are tracked via Dependabot and audited with
`pip-audit` as part of CI. The project's policy on vulnerable transitive dependencies:

- **No patched version available** — isolate the vulnerable package behind an explicit
  opt-in extra so it is never installed by default. Document the risk and provide an
  upgrade path once a fix ships.
- **Patched version available** — pin to the patched version in `pyproject.toml` and
  regenerate `uv.lock`.

#### Known advisory: diskcache ≤ 5.6.3 (CVSS 5.2 — Moderate)

`diskcache` uses Python `pickle` for cache serialization. An attacker with write
access to the cache directory can achieve arbitrary code execution when a victim
application reads from the cache. No patched version exists as of 2026-06.

**Impact on Velune**: Velune does not use `diskcache` directly. The vulnerable package
entered only as a transitive dependency of `llama-cpp-python`. It has been removed
from the default install by splitting the optional extras:

- `pip install velune-cli` — does **not** install `diskcache`
- `pip install 'velune-cli[gguf]'` — does **not** install `diskcache` (GGUF metadata
  reading via the `gguf` library is unaffected)
- `pip install 'velune-cli[all]'` — does **not** install `diskcache`
- `pip install 'velune-cli[llamacpp]'` — **installs `llama-cpp-python` and therefore
  `diskcache`**; only use in a trusted, single-user environment where the cache
  directory cannot be written to by other principals

`pip-audit` reports **no known vulnerabilities** on a default install. This advisory
will be revisited and the `[llamacpp]` restriction lifted once `diskcache` ships a
patched release or `llama-cpp-python` removes the dependency.
