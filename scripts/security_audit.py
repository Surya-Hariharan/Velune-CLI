...
def check_asyncio_run_count(root: Path = _REPO_ROOT, package: str = "velune") -> list[Finding]:
    """Fail if more than one asyncio.run() call exists (P0-1 regression guard).

    The single allowed call lives in velune/kernel/entrypoint.py::run_async().
    """
    findings: list[Finding] = []
    ...
    for py_file in _iter_py_files(root, package):
        ...
        for lineno, raw in enumerate(py_file.read_text(encoding="utf-8").splitlines(), start=1):
            if pattern.search(raw):
                hits.append((rel, lineno, "asyncio.run() call"))

    if len(hits) > 1:
        for hit in hits:
            findings.append((hit[0], hit[1], f"Extra asyncio.run() call — only one permitted (P0-1 regression). Found {len(hits)} total."))
