...
async def test_fresh_workspace_first_turn_persists_to_real_sqlite(tmp_path):
    ...
    try:
        turn_id = await manager.record_turn(
            session_id="ses-1", role="user", content="hello velune", model="m1"
        )
        assert turn_id  # real UUID-derived id, not empty
        assert len(manager.working.get_turns()) == 1

        turns = await manager.episodic_memory.get_session_history("ses-1")
        assert len(turns) == 1
...
async def test_second_turn_coordinator_query_sees_first_turn_in_episodic(tmp_path):
    manager, pool = await _make_manager(tmp_path / "cognitive.db")
    try:
        await manager.record_turn(session_id="ses-1", role="user", content="what is auth flow")
        await manager.record_turn(
            session_id="ses-1", role="assistant", content="auth flow uses JWT tokens"
        )

        context = await manager.retrieve("auth flow", workspace_root="ses-1", budget=4000)
...
async def test_session_restart_turns_survive_new_pool_instance(tmp_path):
    ...
    await manager1.record_turn(session_id="ses-1", role="user", content="remember this")
    ...
    try:
        turns = await manager2.episodic_memory.get_session_history("ses-1")
        assert len(turns) == 1
...
async def test_compaction_does_not_trigger_with_few_turns(tmp_path):
    ...
    manager, pool = await _make_manager(tmp_path / "cognitive.db", provider=provider)
    try:
        await manager.record_turn(session_id="ses-1", role="user", content="just one turn")
