...
def test_dimension_negotiation_768(tmp_path):
    ...
    doc = RetrievalDocument(id="doc1", content="test", embedding=[0.1] * 768, namespace="default")
...
def test_dimension_mismatch_fails_gracefully(tmp_path):
    ...
    # Create with 768-dim
    retriever.upsert(
        RetrievalDocument(id="d1", content="hello", embedding=[0.1] * 768, namespace="default")
    )
...
def test_semantic_memory_tier_validation():
    ...
    results = tier.search_similarity(
        collection_name="test-semantic", query_vector=[0.1] * 768, limit=1
    )
    assert len(results) == 1
