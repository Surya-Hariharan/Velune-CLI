...
def test_system_python_trusted(monkeypatch):
    ...
    if sys.platform == "win32":
        monkeypatch.setattr(
            "shutil.which",
            lambda cmd: "C:\\Windows\\System32\\echo.exe" if cmd == "echo" else shutil.which(cmd),
        )
...
def test_validate_uses_runtime_config_when_no_override_provided(monkeypatch) -> None:
    ...
    from velune.kernel.config import ExecutionConfig, VeluneConfig

    custom_config = VeluneConfig(execution=ExecutionConfig(allowed_executables=["special_command"]))
