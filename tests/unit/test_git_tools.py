...
async def test_git_tool_does_not_block_event_loop(git_repo: Path):
    ...
    await asyncio.gather(loop_monitor(), tool.execute(directory=str(git_repo)))
...
def test_rollback_manager_preserves_uncommitted_changes(git_repo: Path):
    ...
    subprocess.run(
        ["git", "commit", "-m", "Add target"], cwd=git_repo, capture_output=True, check=True
    )
    ...
    checkpoint_state = manager.save_state(
        checkpoint_id="test-cp-1", files_to_track=[Path("src/main.py")]
    )
