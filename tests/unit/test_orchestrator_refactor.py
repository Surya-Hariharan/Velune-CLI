...
async def test_tier_classifier_policy():
    ...
    classifier = TierClassifier(
        task_registry=mock_registry, max_council_tier="full", low_resource_mode=False
    )
...
async def test_style_resolver_async_safety():
    ...
    resolver._scan_directory = MagicMock(
        return_value={
...
async def test_orchestrator_refactored_execution():
    ...
    result = await orchestrator.execute_task(
        prompt="explain authentication logic", repo_context="some context", council_tier="instant"
    )
