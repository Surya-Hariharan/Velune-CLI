    ...
    try:
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="VELUNE_BENCH:latency_ms=1.0\nVELUNE_BENCH:peak_rss_kb=1.0\nNO_USER_SITE_SET\nSTARTUP_CLEARED",
            )
...
async def test_executor_offloads_sandbox_to_thread(temp_workspace):
    ...
    with patch("asyncio.to_thread", new_callable=AsyncMock) as mock_to_thread:
        ...
        plan.steps = [step]
        executor.planner.compile = MagicMock(
            return_value=MagicMock(topological_sort=lambda: [step])
        )
...
async def test_orchestrator_scans_repository_context():
    orchestrator = CouncilOrchestrator(provider_registry=MagicMock(), mapper=MagicMock())
    orchestrator.firewall.scan_file_for_injection = MagicMock(
        return_value={
            "is_safe": False,
            "quarantined": True,
    ...
    mock_repo = MagicMock()
    mock_repo.index.return_value = MagicMock(
        root_path="/tmp", files=[MagicMock(path="test.py", language=MagicMock(value="python"))]
    )
    ...
    with patch("velune.kernel.registry.get_container", return_value=mock_container):
        async for _chunk in orchestrator.stream("prompt"):
            pass
