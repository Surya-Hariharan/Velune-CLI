...
class TestMakeConnection:
    ...
    def test_websocket_transport_supported(self):
        # WebSocket is a supported transport; the factory returns a connection.
        from velune.mcp.transports.websocket import WebSocketConnection

        cfg = ServerConfig(name="x", transport=TransportType.WEBSOCKET, url="ws://localhost")
        assert isinstance(make_connection(cfg), WebSocketConnection)
