...
Compatible with Claude Code's ``.claude-plugin/plugin.json`` format so that
CC plugins (with commands/agents/skills/hooks) can be dropped into
``~/.velune/plugins/`` and discovered automatically.
"""
...
class DeclarativePluginManifest:
    ...
    def from_dict(cls, data: dict[str, Any], root: Path) -> "DeclarativePluginManifest":
        ...
        known = {
            ...
            "repository", "license", "keywords", "enabled",
    ...
    def load(cls, plugin_root: Path) -> "DeclarativePluginManifest | None":
        ...
        1. ``<root>/.velune-plugin/plugin.json``   (Velune native)
        2. ``<root>/.claude-plugin/plugin.json``   (CC-compatible)
        3. ``<root>/plugin.json``                  (flat layout)
        """
        candidates = [
