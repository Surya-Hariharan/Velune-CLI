...
async def _chat_command_async(cli_context: CLIContext, resume_id: str | None = None) -> None:
    ...
    _system_messages = [
        ...
        {
            "role": "system",
            "content": f"Workspace file index (paths only — use !read to get content):\n{formatted_snap}",
        },
    ]
    ...
    if resume_id:
        ...
        if loaded is None:
            console.print(
                f"[yellow]Session '{resume_id}' not found — starting a new session.[/yellow]"
            )
            ...
            try:
                ...
                def _text_tree(
                    ...
                    for item in items:
                        if _sc.is_ignored(item):
                            continue
                        _lines.append(prefix + item.name + ("/" if item.is_dir() else ""))
                        if item.is_dir():
                            _text_tree(item, prefix + "  ", d + 1, _max_d, _sc, _lines)
