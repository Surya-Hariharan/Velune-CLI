...
def session_list(
    ...
    console.print()
    console.print(
        f"[{design.MUTED}]Resume a session:[/] [bold]velune chat --session <id>[/bold]"
    )
    console.print(
        f"[{design.MUTED}]Delete a session: [/] [bold]velune session delete <id>[/bold]"
    )
...
def session_delete(
    ...
    if not yes:
        typer.confirm(
            f"Delete session '{meta.title}' ({session_id})?", default=False, abort=True
        )
