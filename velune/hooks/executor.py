...
Design principles (same as Claude Code):
...
async def run_hook(
    ...
    if workspace:
        ...
        proc_env["CLAUDE_CODE_DIR"] = str(workspace)  # compatibility alias
    proc_env.update(defn.env)
    if env_overrides:
        proc_env.update(env_overrides)

    # Resolve the command — support both ``python3 script.py`` style and
    # simple executables. On Windows use shell=True; on POSIX split manually.
    cmd = defn.command
...
def build_post_tool_payload(
    ...
    return {
        ...
        "tool_name": tool_name,
        "tool_input": tool_input,
        "tool_result": tool_result if isinstance(tool_result, (dict, list, str, int, float, bool)) else str(tool_result),
        "session_id": session_id,
        "workspace": workspace,
    }
