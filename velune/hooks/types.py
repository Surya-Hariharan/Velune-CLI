...
Hook events mirror Claude Code's lifecycle boundaries so that hooks written
for either tool are largely interchangeable.
"""
...
class HookCondition:
    ...
    Format mirrors CC: ``Bash(rm -rf*)``, ``Edit(src/**)`` or plain ``*``.
